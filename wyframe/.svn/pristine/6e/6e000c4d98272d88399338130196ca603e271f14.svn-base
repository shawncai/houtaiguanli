...
Xgt_goods_store_bill.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
       	{title: '领用单ID', field: 'bill_out_id', visible: false, align: 'center', valign: 'middle'},
...
,
        {title: '领用单描述', field: 'bill_dsc', align: 'center', valign: 'middle', sortable: true}
,
...
Xgt_goods_store_bill.openAddXgt_goods_store_bill = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/xgt_goods_store_bill_collar/xgt_goods_store_bill_add'
    });
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail_update = function () {
    if (this.check()&&Xgt_goods_store_bill.seItem.st_id=="编辑") {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill_collar/xgt_goods_store_bill_edit/' + Xgt_goods_store_bill.seItem.bill_out_id
        });
        this.layerIndex = index;
    }else{
        Feng.error("已提交不能修改");
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail_review = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill_collar/xgt_goods_store_bill_review/' + Xgt_goods_store_bill.seItem.bill_out_id
        });
        this.layerIndex = index;
    }
};
...
Xgt_goods_store_bill.delete = function () {
    if (this.check()&&Xgt_goods_store_bill.seItem.aud_st_id=="待审核") {
     var operation = function(){
        var ajax = new $ax(Feng.ctxPath + "/xgt_goods_store_bill_collar/delete", function (data) {
            Feng.success("删除成功!");
        ...
        ajax.start();
    	 ...
    	 Feng.confirm("是否刪除该领用单?", operation);
    }else{
        Feng.error("删除失败！");
    }
};
...
$(function () {
    ...
    var table = new BSTable(Xgt_goods_store_bill.id, "/xgt_goods_store_bill_collar/list", defaultColunms);
...
Xgt_goods_store_bill.export = function () {
       window.location.href=Feng.ctxPath + "/xgt_goods_store_bill_collar/export";
};
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill_collar/xgt_goods_store_bill_detail/' + Xgt_goods_store_bill.seItem.bill_out_id
        });
        this.layerIndex = index;
    }
};
...
Xgt_goods_store_bill.add = function () {
     ...
     var operation = function(){
        var ajax = new $ax(Feng.ctxPath + "/xgt_goods_store_bill_collar/add", function (data) {
            Feng.success("添加成功!");
            Xgt_goods_store_bill.table.refresh();
        }, function (data) {
        ...
        ajax.start();
    	 ...
    	 Feng.confirm("是否添加该领用单?", operation);
};
