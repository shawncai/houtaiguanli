...
Xgt_goods_store_bill.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
       	{title: '入库单ID', field: 'bill_in_id', visible: false, align: 'center', valign: 'middle'},
        {title: '类型', field: 'in_bill_type', align: 'center', valign: 'middle', sortable: true}
,
        {title: '批次号', field: 'bill_no', align: 'center', valign: 'middle', sortable: true}
,
        {title: '入库单描述', field: 'bill_dsc', align: 'center', valign: 'middle', sortable: true}
,
...
Xgt_goods_store_bill.openAddXgt_goods_store_bill = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/xgt_goods_store_bill/xgt_goods_store_bill_add'
    });
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail_update = function () {
    if (this.check()&&Xgt_goods_store_bill.seItem.st_id=="编辑") {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill/xgt_goods_store_bill_change/' + Xgt_goods_store_bill.seItem.bill_in_id
        });
        this.layerIndex = index;
    }else{
        Feng.error("已提交不能修改");
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail_review = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill/xgt_goods_store_bill_review/' + Xgt_goods_store_bill.seItem.bill_in_id
        });
        this.layerIndex = index;
    }
};
...
Xgt_goods_store_bill.openXgt_goods_store_billDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xgt_goods_store_bill/xgt_goods_store_bill_detail/' + Xgt_goods_store_bill.seItem.bill_in_id
        });
        this.layerIndex = index;
    }
};
...
Xgt_goods_store_bill.delete = function () {
    if (this.check()&&Xgt_goods_store_bill.seItem.aud_st_id=="待审核") {
     var operation = function(){
        var ajax = new $ax(Feng.ctxPath + "/xgt_goods_store_bill/delete", function (data) {
            Feng.success("删除成功!");
            Xgt_goods_store_bill.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("id",Xgt_goods_store_bill.seItem.bill_in_id)
        ajax.start();
    }
...
$(function () {
    ...
    var table = new BSTable(Xgt_goods_store_bill.id, "/xgt_goods_store_bill/list", defaultColunms);
...
Xgt_goods_store_bill.export = function () {
       window.location.href=Feng.ctxPath + "/xgt_goods_store_bill/export";
};
...
Xgt_goods_store_bill.collectData = function() {
    this .set('bill_in_id') .set('in_bill_type') .set('bill_no') .set('bill_dsc') .set('cpn_store_id') .set('cpn_id') .set('cpn_branch_id') .set('cre_dt') .set('oper_user').set('aud_st_id').set('aud_user');
}
...
Xgt_goods_store_bill.add = function () {
     ...
     var operation = function(){
        ...
        ajax.start();
    	 ...
    	 Feng.confirm("是否添加该入库单?", operation);
};
