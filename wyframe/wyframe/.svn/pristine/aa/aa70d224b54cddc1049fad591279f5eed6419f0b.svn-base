package wy.addons.zcgl.inStore.controller;

import com.alibaba.fastjson.JSON;
import wy.addons.zcgl.inStore.dao.Xgt_goods_store_billDao;
...
public class Xgt_goods_store_billController extends BaseController {
    ...
    public String xgt_goods_store_bill_change(@PathVariable Integer xgt_goods_store_bill_in_Id, Model model) {
        Xgt_goods_store_bill xgt_goods_store_bill = this.xgt_goods_store_billDao.queryById(xgt_goods_store_bill_in_Id);
        model.addAttribute(xgt_goods_store_bill);
        LogObjectHolder.me().set(xgt_goods_store_bill);
    ...
    public String xgt_goods_store_billUpdate(@PathVariable Integer xgt_goods_store_billId, Model model) {
        Xgt_goods_store_bill xgt_goods_store_bill = this.xgt_goods_store_billDao.queryById(xgt_goods_store_billId);
        model.addAttribute(xgt_goods_store_bill);
        LogObjectHolder.me().set(xgt_goods_store_bill);
    ...
    public Object list(@RequestParam(required = false) String name,@RequestParam(required = false)Integer st_id,@RequestParam(required = false)Integer aud_st_id) {
        ...
        if (userId.equals(Const.ADMIN_ID)){
            ...
            List<Map<String, Object>> list = this.xgt_goods_store_billDao.selectOtherOrders(name,st_id,aud_st_id,userId);
            return super.warpObject(new Xgt_goods_store_billWarpper(list));
        }
    }
...
//        map.put("in_bill_type",11);// '入库单据类型: 10：采购入库单；11：其它入库单；12：调拨入库单；'
    ...
    public Object review(@RequestParam Integer id, @RequestParam String idata, @RequestParam String tdata) {
        Integer userId = ShiroKit.getUser().getId();
        xgt_goods_store_billDao.updateState(id, userId);
        ...
        int cpnBrandId = managerDao.queryCpnBrandId(userId);
        for (Map map : list) {
            map.putAll(inputMap);
            map.put("cpn_id",cpnId);
            map.put("cpn_branch_id",cpnBrandId);
    ...
    public Object delete(@RequestParam Integer id) {
        Integer userId = ShiroKit.getUser().getId();
        xgt_goods_store_billDao.deleteState(id,userId);
        return SUCCESS_TIP;
    }
    ...
    public void export(HttpServletResponse res) throws ParsePropertyException, InvalidFormatException, IOException {
        ...
        List<Map<String, Object>> list = this.xgt_goods_store_billDao.list(condition);
...
//			varList = xgt_goods_store_billDao.quertyMatDistributors();
//
//			System.err.println("随机匹配给matDistribution");
//			System.err.println("varList" + varList.toString());
//
//			// 查询所有配送商相关联的配送区域
//			List<Map<String,Object>> listAllChannel = xgt_goods_store_billDao.listAllChannelArea(null);
//
...
//					e.printStackTrace();
//				}
//				xgt_goods_store_billDao.updateSendChnlId(param);
//			}
//		} catch (Exception e) {
//		}
//	}
