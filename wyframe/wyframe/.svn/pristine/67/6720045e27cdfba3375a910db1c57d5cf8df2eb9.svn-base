...
Xyd_cpn.search = function () {
    ...
    console.log(Cpn_cata_paridDlg.cpn_cata_id);
    Xyd_cpn.table.refresh({query: queryData});
};

$(function () {
	var defaultColunms = Xyd_cpn.initColumn();
    var table = new BSTable(Xyd_cpn.id, "/xyd_cpn/list" , defaultColunms);
...
Xyd_cpn.openXyd_cpnDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/xyd_cpn/xyd_cpn_detail/' + Xyd_cpn.seItem.cpn_id
        });
        this.layerIndex = index;
    }
};
