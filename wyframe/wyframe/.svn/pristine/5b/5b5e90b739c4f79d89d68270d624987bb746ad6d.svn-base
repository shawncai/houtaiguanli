...
Xgt_goods_store_billInfoDlg.collectData = function() {
	this.set('in_bill_type').set('store').set('bill_in_id').set('in_bill_type').set('bill_no').set('bill_dsc').set('cpn_store_id').set('cre_dt').set('st_id').set('oper_user');
}
...
Xgt_goods_store_billInfoDlg.reviewSubmit = function() {
	...
	// 提交信息
	var ajax = new $ax(Feng.ctxPath + "/xgt_goods_store_bill/review/", function(
			data) {
		Feng.success("审核成功!");
		window.parent.Xgt_goods_store_bill.table.refresh();
	}, function(data) {
