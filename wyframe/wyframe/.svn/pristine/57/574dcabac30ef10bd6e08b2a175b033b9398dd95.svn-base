package wy.addons.zcgl.collar.controller;

import com.alibaba.fastjson.JSON;
import wy.addons.zcgl.collar.dao.Xgt_goods_store_bill_collarDao;
 ...
 */
@Controller
@RequestMapping("/xgt_goods_store_bill_collar")
public class Xgt_goods_store_bill_collarController extends BaseController {
    private String PREFIX = "/addons/zcgl/collar/";
    @Resource
    Xgt_goods_store_bill_collarDao xgt_goods_store_bill_collarDao;
    @Resource
    ...
    model.addAttribute("timer", DateUtil.format(new Date() ,"yyyy-MM-dd HH:mm:ss"));
        return PREFIX + "xgt_goods_store_bill_collar.html";
    }
    ...
    public String xgt_goods_store_bill_add_param(@PathVariable String strs) {
        ...
        return PREFIX + "xgt_goods_store_bill_collar_stroe_add_param.html";
    }
    ...
    public String xgt_goods_store_billUpdate(@PathVariable Integer xgt_goods_store_billId, Model model) {
        Xgt_goods_store_bill_collar xgt_goods_store_bill_collar = this.xgt_goods_store_bill_collarDao.queryById(xgt_goods_store_billId);
        model.addAttribute(xgt_goods_store_bill_collar);
        LogObjectHolder.me().set(xgt_goods_store_bill_collar);
    ...
    public String xgt_goods_store_billDetail(@PathVariable Integer xgt_goods_store_billId, Model model) {
        Xgt_goods_store_bill_collar xgt_goods_store_bill_collar = this.xgt_goods_store_bill_collarDao.queryById(xgt_goods_store_billId);
        xgt_goods_store_bill_collar.setCre_dt(DateUtil.format(xgt_goods_store_bill_collar.getCre_dt()));
        model.addAttribute(xgt_goods_store_bill_collar);
        LogObjectHolder.me().set(xgt_goods_store_bill_collar);
    ...
    public Object list(@RequestParam(required = false) String name,@RequestParam(required = false)Integer st_id,@RequestParam(required = false)Integer aud_st_id) {
        ...
        if (userId.equals(Const.ADMIN_ID)){
            List<Map<String, Object>> list = this.xgt_goods_store_bill_collarDao.selectOrders(name,st_id,aud_st_id);
            return super.warpObject(new Xgt_goods_store_billWarpper(list));
        }
       else {
            List<Map<String, Object>> list = this.xgt_goods_store_bill_collarDao.selectOtherOrders(name,st_id,aud_st_id,userId);
            return super.warpObject(new Xgt_goods_store_billWarpper(list));
        }
    }
    ...
    public Object getInStoreNo() {
        ...
        int a = Integer.parseInt(xgt_goods_store_bill_collarDao.getCreateTimeCount(startTime))+1;
    ...
    public Object add(@RequestParam String iData, @RequestParam String tData) {
        ...
        map.put("out_bill_type", 20);//'单据类型  20：领用单；21：借用单 22：其他出库单',
...
//        map.put("in_bill_type",11);// '入库单据类型: 10：采购入库单；11：其它入库单；12：调拨入库单；'
    ...
    public Object update(Xgt_goods_store_bill xgt_goods_store_bill, @RequestParam String str) {
        ...
        this.xgt_goods_store_bill_collarDao.deleteDetl(id);
        ...
        xgt_goods_store_bill_collarDao.insertDetail(map2);
    ...
    public Object keep(@RequestParam String iData, @RequestParam Integer bill_out_id) {
        ...
        map.put("shop_num", shop_num);
        this.xgt_goods_store_bill_collarDao.updateTotal(shop_num, bill_out_id);
        this.xgt_goods_store_bill_collarDao.deleteDetail(bill_out_id);
        this.xgt_goods_store_bill_collarDao.insertDetail(list);
        return SUCCESS_TIP;
    }
    ...
    public Object submit(@RequestParam String iData, @RequestParam Integer bill_out_id) {
        ...
        this.xgt_goods_store_bill_collarDao.deleteDetail(bill_out_id);
        this.xgt_goods_store_bill_collarDao.insertDetail(list);
        this.xgt_goods_store_bill_collarDao.updateSubmit(bill_out_id);
        return SUCCESS_TIP;
    }
    /**
     * 审核出库单
    ...
    public Object review(@RequestParam Integer id) {
        Integer userId = ShiroKit.getUser().getId();
        xgt_goods_store_bill_collarDao.updateInState(id,userId);
        return super.SUCCESS_TIP;
    }
    /**
     * 删除入库单
    ...
    public Object delete(@RequestParam Integer id) {
        Integer userId = ShiroKit.getUser().getId();
        xgt_goods_store_bill_collarDao.deleteState(id,userId);
        return SUCCESS_TIP;
    }
    ...
    public void export(HttpServletResponse res) throws ParsePropertyException, InvalidFormatException, IOException {
        ...
        List<Map<String, Object>> list = this.xgt_goods_store_bill_collarDao.list(condition);
...
//			varList = xgt_goods_store_bill_collarDao.quertyMatDistributors();
//
//			System.err.println("随机匹配给matDistribution");
//			System.err.println("varList" + varList.toString());
//
//			// 查询所有配送商相关联的配送区域
//			List<Map<String,Object>> listAllChannel = xgt_goods_store_bill_collarDao.listAllChannelArea(null);
//
...
//					e.printStackTrace();
//				}
//				xgt_goods_store_bill_collarDao.updateSendChnlId(param);
//			}
//		} catch (Exception e) {
//		}
//	}
