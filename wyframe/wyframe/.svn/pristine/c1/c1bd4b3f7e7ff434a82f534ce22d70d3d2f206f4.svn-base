...
public class UserMgrController extends BaseController {
     ...
     * 跳转到用户企业分配页面
     */
    @RequestMapping(value = "/user_allocation/{userId}")
    public String allocation(@PathVariable Integer userId, Model model){
        model.addAttribute("userId",userId);
        return PREFIX + "user_allocation.html";
    }
    /**
   	 * 点击跳转用户区域分配管理
   	 */
...
//    @Permission(Const.ADMIN_NAME)
    ...
    @ResponseBody
    public Tip setDept(@RequestParam("userId") Integer userId, @RequestParam("deptIds") String deptIds){
        this.managerDao.setDepts(userId, deptIds);
        Integer deptid=managerDao.selectdeptId(userId);
        System.err.println(deptid);
        Integer cpnId=managerDao.selectCpn(deptid);
        Integer cpnBranchId=managerDao.selectCpnBrand(deptid);
        System.err.println(cpnId);
        System.err.println(cpnBranchId);
        this.managerDao.setCpnAndCpnBranch(deptid,cpnBranchId,cpnId);
        this.managerDao.setUserDept(userId, deptIds);
        this.managerDao.setUserCpnAndCpnBranch(deptid,cpnBranchId,cpnId);
	    return SUCCESS_TIP;
    }
    /**
     * 用户企业分配
    ...
    public List<ZTreeNode>DeptTreeListByUserId(@PathVariable Integer userId){
        List<Integer> list = managerDao.selectDeptIdById(userId);
        String deptid = "";
        ...
        if (userRoleId.equals(Const.ADMIN_ROLE_ID)){
            if(ToolUtil.isEmpty(deptid)){
                List<ZTreeNode>deptTreeList=this.managerDao.deptTreeList();
                return  deptTreeList;
            }
            else {
                String[] strArray=Convert.toStrArray(",",deptid);
                List<ZTreeNode>deptTreeListByUserId=this.managerDao.deptTreeListById(strArray,userId);
                return  deptTreeListByUserId;
            }
        }
        else {
            if (ToolUtil.isEmpty(deptid)) {
                List<ZTreeNode> deptTreeList = this.managerDao.deptSomeTreeList(userNowId);
                return deptTreeList;
            } else {
                System.err.println(deptid);
                String[] strArray = Convert.toStrArray(",", deptid);
                List<ZTreeNode> deptTreeListByUserId = this.managerDao.deptSomeTreeListById(strArray, userNowId ,userId);
                return deptTreeListByUserId;
            }
        }
    }
...
//			file.transferTo(new File(fileSavePath + pictureName));
    ...
    public String buildJsonTree(HttpServletRequest request) {
        ...
        Integer cpnId = managerDao.queryCpnId(userId);
        if(ShiroKit.isAdmin()){
            List<Node> nodes = managerDao.tree1();
            String json = new TreeBuilder().buildTree(nodes);
            return json;
        }
        else {
            List<Node> nodes = managerDao.tree2(cpnId);
            String json = new TreeBuilder().buildTree(nodes);
            return json;
        }
    }
    ...
    @ResponseBody
    public JSONObject buildJson(HttpServletResponse response, @RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime,@RequestParam(required = false) Integer id) {
        Integer userId = ShiroKit.getUser().getId();
        Integer cpnId = managerDao.queryCpnId(userId);
        if (null==id){
            id=0;
        ...
        if(ShiroKit.isAdmin()){
            List list=managerDao.list4(name,beginTime,endTime,id);
            ...
            List list=managerDao.list5(cpnId,name,beginTime,endTime,id);
            Map<String,Object> map = new HashMap<String,Object>();
            map.put("data",list);
            String json= JSON.toJSONString(map);
            System.out.println(json);
    ...
    @ResponseBody
    public ResultMap<List<Map>> backContent(Page page, @RequestParam("limit") int limit, @RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime,@RequestParam(required = false) Integer id){
        Integer userId = ShiroKit.getUser().getId();
	    Integer cpnId = managerDao.queryCpnId(userId);
        ...
        if(ShiroKit.isAdmin()) {
            ...
            List<Map> contentList = managerDao.selectPageList(page, start, rows, name,beginTime,endTime,id);
            int totals = managerDao.selectPageCount(page, id);
            ...
            List<Map> contentList = managerDao.selectPageList2(cpnId,page, start, rows, name,beginTime,endTime, id);
            int totals = managerDao.selectPageCount(page, id);
            page.setTotalRecord(totals);
            return new ResultMap<List<Map>>("", contentList, 0, totals);
        }
