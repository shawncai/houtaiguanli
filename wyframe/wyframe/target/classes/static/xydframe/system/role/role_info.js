...
RolInfoDlg.onClickPName = function (e, treeId, treeNode) {
    ...
    RolInfoDlg.pName=RolInfoDlg.zTreeInstance.getSelectedVal();
    RolInfoDlg.pid=treeNode.id;
};
...
RolInfoDlg.addSubmit = function () {
    ...
    var ajax = new $ax(Feng.ctxPath + "/role/add", function (data) {
        Feng.success("添加成功!");
        var ajax1 = new $ax(Feng.ctxPath + "/role/list", function(data){
            tableData=data
        });
        ajax1.start();
        window.parent.layui.table.reload('roleTable', {data:tableData});
