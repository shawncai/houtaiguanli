...
public class Xgt_goods_productsController extends BaseController {
    ...
    public String buildJsonTree(HttpServletRequest request) {
        List<Node> nodes = xgt_goods_productsDao.tree1();
       /* for (int i = 0; i < nodes.size(); i++) {
           Node node1 = new Node();
           node1.setName(nodes.get(i).getName());
           node1.setId(nodes.get(i).getId());
           node1.setPid(nodes.get(i).getPid());
           node1.setChildren(nodes.get(i).getChildren());
           nodes.add(node1);
        }*/
    ...
    public JSONObject buildJson(HttpServletResponse response,@RequestParam(required = false) String condition,@RequestParam(required = false) Integer id) {
        ...
        List list=xgt_goods_productsDao.list4(condition,id);
    ...
    @ResponseBody
    public ResultMap<List<Map>> backContent(Page page, @RequestParam("limit") int limit,@RequestParam(required = false) Integer id){
        ...
        List<Map>contentList=xgt_goods_productsDao.selectPageList(page,start,rows,id);
        int totals=xgt_goods_productsDao.selectPageCount(page,id);
     ...
     * @return
     */
    @RequestMapping("/backContent1/{id}")
    @ResponseBody
    public ResultMap<List<Map>> backContent1(Page page, @RequestParam("limit") int limit,@RequestParam(required = false) Integer id){
        System.out.println("backContent1========================"+limit);
        page.setRows(limit);
        System.out.println("page:"+page.toString());
        Integer start=page.getStart();
        Integer rows=page.getRows();
    ...
    @RequestMapping("/backContent2")
    @ResponseBody
    public ResultMap<List<Map>> backContent2(Page page, @RequestParam("limit") int limit,@RequestParam(required = false) Integer id){
        System.out.println("backContent========================"+limit);
        page.setRows(limit);
        System.out.println("page:"+page.toString());
        Integer start=page.getStart();
