...
Cpn_cata_paridDlg.onClickCpn = function(e, treeId, treeNode) {
	$("#cpnPname").attr("value",
			Cpn_cata_paridDlg.cpnzTreeInstance.getSelectedVal());
	$("#cpn_cata_id").attr("value", treeNode.id);
	Cpn_cata_paridDlg.cpnPname = Cpn_cata_paridDlg.cpnzTreeInstance
			.getSelectedVal();
...
Sys_areaInfoDlg.onClickArea = function(e, treeId, treeNode) {
	$("#codeName")
			.attr("value", Sys_areaInfoDlg.zTreeInstance.getSelectedVal());
...
Sys_areaInfoDlg2.onClickArea2 = function(e, treeId, treeNode) {
	$("#workName").attr("value",
			Sys_areaInfoDlg2.zTreeInstance2.getSelectedVal());
	$("#work_area_id").attr("value", treeNode.id);
	Sys_areaInfoDlg2.workName = Sys_areaInfoDlg2.zTreeInstance2
			.getSelectedVal();
...
Xyd_cpnInfoDlg.set = function(key, val) {
	this.xyd_cpnInfoData[key] = (typeof value == "undefined") ? $("#" + key)
			.val() : value;
	return this;
}
...
Xyd_cpnInfoDlg.editSubmit = function() {
	...
	// 提交信息
	var ajax = new $ax(Feng.ctxPath + "/xyd_cpn/update", function(data) {
		Feng.success("修改成功!");
		window.parent.Xyd_cpn.table.refresh();
		Xyd_cpnInfoDlg.close();
	}, function(data) {
...
Xyd_cpnInfo.collectData = function() {
	this.set('cpn_id').set('cpn_cata_id').set('cpn_code').set('cpn_nm').set(
			'cpn_main_nm').set('cpn_main_phone').set('cpn_main_mail').set(
			'work_area_id').set('work_address').set('code_area_id').set(
			'code_address').set('cpn_acc_nm').set('cpn_acc_phone').set(
			'cpn_acc_mail').set('cpn_acc_card_no').set('cpn_acc_card_url1')
			.set('cpn_acc_card_url2').set('cpn_code_url').set('cpn_st').set(
					'cp_sp_st').set('sp_user_id').set('sale_user_id');
}
